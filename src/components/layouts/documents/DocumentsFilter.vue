  ...
  <div class="border-bottom py-1 d-flex w-100">
    ...
    >
      ...
      <SelectComponent
        ...
        :options="museumsArray"
        :model-value="filterProperties.museum"
        :default-value="'Museum'"
      ...
      <ButtonWhiteComponent
        class="col-2 btn"
        type="submit"
        style="width: 10% !important; height: 48px"
        :label="'Cancel'"
        @click="clear"
      />
      <ButtonWhiteComponent
...
const { filter, clearFilter, getMuseums, museums, getTourList } =
  useDocuments();
...
const museumsArray = computed(() => museums.value);
const view = ref('list view');

const setMuseum = (val) => {
  filterProperties.value.museum = val;
  sendFilter();
};
...
const sendFilter = () => {
  filter(filterProperties.value.museum, filterProperties.value.date_range);
  getTourList();
};
